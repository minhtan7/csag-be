...
formController.matchingReceiver = async (req, res, next) => {
	try {
		...
		utilsHelper.sendResponse(res, 200, true, { sortingReceiver, totalPage, totalReceiver, page, limit }, null, 'Matching form');
	} catch (err) {
		next(err);
	}
};

formController.getFormByUser = async (req, res, next) => {
	try {
		const id = req.params.id;

		let form = await Form.findOne({ userId: id }).populate('userId').lean();
		console.log(form);
		res.status(200).json({
			success: true,
			data: { form },
